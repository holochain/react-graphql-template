...
pub fn validate_entry_create(entry: NoteEntry, validation_data: hdk::ValidationData) -> Result<(), std::string::String> {
...
pub fn validate_entry_modify(new_entry: NoteEntry, old_entry: NoteEntry, old_entry_header: ChainHeader, validation_data: hdk::ValidationData) -> Result<(), std::string::String> {
    ...
    if let Some(p) = validation_data.package.chain_header.provenances().get(0) {
        if AGENT_ADDRESS.to_string() == p.0.to_string() {
          Ok(())
        }
        else {
...
pub fn validate_entry_delete(old_entry: NoteEntry, old_entry_header: ChainHeader, validation_data: hdk::ValidationData) -> Result<(), std::string::String> {
...
pub fn validate_link_add(link: LinkData, validation_data: hdk::ValidationData) -> Result<(), std::string::String> {
    hdk::debug(format!("validate_link_add_link: {:?}", link)).ok();
    hdk::debug(format!("validate_link_add_validation_data: {:?}", validation_data)).ok();
    Ok(())
}

pub fn validate_link_remove(link: LinkData, validation_data: hdk::ValidationData) -> Result<(), std::string::String> {
